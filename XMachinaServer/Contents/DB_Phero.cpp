...
void DB_Phero::FetchDataFromDataBase(const wchar_t* query)
{
    ...
    while ((mSQL_Ret = SQLFetch(mSQL_hStmt)) == SQL_SUCCESS || mSQL_Ret == SQL_SUCCESS_WITH_INFO) {
        SQLGetData(mSQL_hStmt, 1, SQL_C_SLONG, &Level, 0, nullptr);
...
void DB_Phero::FetchPheroData(int level)
{
    ...
    std::wcout << L"Level: " << Level << L", LifeTime_second: " << LifeTime_second << L", Amount: " << Amount << std::endl;
}